...
<body class="font-sans text-slate-300 overflow-hidden"> <div class="flex h-screen overflow-hidden relative"> <div class="no-print h-full shrink-0 border-r border-slate-800/50 shadow-2xl z-50">
            <?php include "../include/sidebar_admin.php" ?>
        </div>

        <main class="flex-1 min-w-0 h-full overflow-y-auto custom-scroll main-content bg-slate-950/20">
            
            <div class="lg:hidden bg-slate-900/80 backdrop-blur-md border-b border-slate-800 px-4 py-3 flex justify-between items-center sticky top-0 z-40">
                <span class="text-white font-black uppercase tracking-tighter text-sm">BookLoan <span class="text-rose-500">Log</span></span>
            ...
            <div class="p-4 md:p-10 lg:p-12">
                ...
                <div class="hidden lg:block bg-slate-800/40 rounded-[2.5rem] border border-slate-700/50 overflow-hidden shadow-2xl backdrop-blur-sm">
                    <table class="w-full text-left text-sm">
                        <thead class="bg-slate-900/50 text-slate-400 uppercase text-[10px] font-black tracking-[0.2em]">
                            <tr>
                                ...
                                <th class="px-8 py-5">Waktu Peminjaman</th>
                        ...
                        <tbody class="divide-y divide-slate-700/30 text-slate-400">
                            ...
                            <tr class="hover:bg-slate-700/20 transition-colors">
                                <td class="px-8 py-5">
                                    ...
                                    <div class="text-[9px] text-slate-500 font-mono tracking-tighter uppercase">TR-<?= str_pad($data['id_transaksi'], 4, '0', STR_PAD_LEFT) ?></div>
                                </td>
                                <td class="px-8 py-5 text-xs font-medium max-w-[200px] truncate"><?= $data['judul_buku'] ?></td>
                                <td class="px-8 py-5">
                                    <div class="text-[10px] font-bold text-slate-300"><?= date('d/m/Y', strtotime($data['tanggal_pinjam'])) ?></div>
                                    <div class="text-[9px] mt-1 <?= (strtotime(date('Y-m-d')) > strtotime($data['jatuh_tempo']) && $data['status'] == 'dipinjam') ? 'text-rose-500 font-black italic animate-pulse' : 'text-slate-500 font-bold' ?>">
                                ...
                                <td class="px-8 py-5 text-right">
                                    <?php if($data['status'] == 'dipinjam'): ?>
                                        <a href="?aksi=kembali&id=<?= $data['id_transaksi'] ?>&buku=<?= $data['id_buku'] ?>" 
                                           ...
                                           class="inline-block bg-rose-500 hover:bg-rose-600 text-white text-[9px] font-black py-2 px-5 rounded-xl transition-all shadow-lg shadow-rose-900/20 uppercase tracking-widest">
                                           Selesaikan
                                        </a>
                                    <?php else: ?>
                                        <span class="text-slate-600 text-[10px] font-black uppercase tracking-widest inline-flex items-center gap-2">
                ...
                <div class="lg:hidden space-y-4">
                    ...
                    <div class="bg-slate-800/60 rounded-[1.5rem] border border-slate-700/50 p-5 relative overflow-hidden group">
                        <div class="flex justify-between items-start mb-4">
                            ...
                            <?php if($data['status'] == 'dipinjam'): ?>
                                <span class="bg-orange-500/10 text-orange-400 px-2 py-1 rounded text-[8px] font-black border border-orange-500/20 uppercase tracking-tighter">Dalam Pinjaman</span>
                            <?php else: ?>
                                <span class="bg-emerald-500/10 text-emerald-400 px-2 py-1 rounded text-[8px] font-black border border-emerald-500/20 uppercase tracking-tighter">Selesai</span>
                            <?php endif; ?>
                        </div>

                        <div class="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">Judul Buku</div>
                        <div class="text-xs text-slate-300 mb-4 italic line-clamp-2"><?= $data['judul_buku'] ?></div>

                        <div class="flex justify-between items-end border-t border-slate-700/50 pt-4">
                            <div>
                                <div class="text-[8px] font-black text-slate-500 uppercase tracking-widest">Jatuh Tempo</div>
                            ...
                            <?php if($data['status'] == 'dipinjam'): ?>
                                <a href="?aksi=kembali&id=<?= $data['id_transaksi'] ?>&buku=<?= $data['id_buku'] ?>" 
                                   class="bg-rose-500 text-white text-[9px] font-black py-2.5 px-4 rounded-xl uppercase tracking-widest shadow-lg shadow-rose-900/20 active:scale-95 transition-transform">
                                   Kembalikan
                                </a>
                            <?php else: ?>
                                <div class="flex items-center gap-1.5 text-emerald-500 opacity-50">
                                    <i class="fa-solid fa-check-double text-xs"></i>
                                    <span class="text-[9px] font-black uppercase tracking-widest">Clear</span>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
